 ...
 * 		phoneNumber		String
 * 		address			String
 * @objects A typical Customer is c = <a, b, c, d>, where
 * 										id(a), name(b), phoneNumber(c), address(d)
 * @abstract_properties
 * 		mutable(id) = false			/\ optional(id)= false			/\min(id)=1			/\max(id)=10^9
 * 		mutable(name) = true		/\ optional(name) = false		/\length(name)=50
 * 		mutable(phoneNumber) = true /\ optional(phoneNumber)= true	/\length(phoneNumber)=10
