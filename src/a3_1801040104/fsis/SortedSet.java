 ...
 * @attributes Customer Comparable<Object> Vector<Object>
...
//				  Comparable o2 = (Comparable)gets()[j+1];
	...
	public boolean repOK() {
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			/*
			 * omitted due to the use of generic if (!(x instanceof Comparable)) return
			 * false;
			 */
	 ...
	 *     the elements of this in sequence.
	 *          </pre>
	 *
	 ...
	 * @attribute ind Integer int
	 * @object A typical SortedSetGen is SortedSetGen {currentElements, elements}
	 *         where currentElements(currentElements), elements(elements)
	 * @abstract_properties mutable(ind) = false /\ optional(ind) = false /\
	 *                      min(ind) = 0 /\
	 */
	private class SortedSetGen implements Iterator<Comparable> {
		...
		public Comparable next() throws NoSuchElementException {
			if (hasNext()) {
				Comparable next = getElements()[ind];
				ind++;
				return next;
			}
