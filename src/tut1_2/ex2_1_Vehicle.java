...
public class ex2_1_Vehicle {
  ...
  @DomainConstraint(type ="String",mutable = false, min=ZERO_PLUS,max = 12, optional = false)
   ...
   *          </pre>
   */
  public ex2_1_Vehicle(@AttrRef("name") String n, 
  ...
  /**
   * @effects return this.registration 
   */
  @DOpt(type= OptType.Observer)@AttrRef("registration")
 ...
 * @effects <pre>
   ...
   *              return false</pre> 
   */
  protected boolean validateDimension(double v) {
    if (v <= 0) 
      return false;
    else
      return true;
  ...
  public void travel(String start, String end, int numberOfSeats) {
	  System.out.printf("This vehicle: %s is going from %s to %s, with number of seats are %d"
              , toString(), start, end, numberOfSeats);
  }
}
